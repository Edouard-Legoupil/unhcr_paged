...
The cookbook covers basic use cases. If one has to deal to with complex survey datasset, based on a population sample, we are advising to look at the dedicated package [KoboloadeR](https://unhcr.github.io/koboloadeR/docs/).
 ...
 * It allows for peer review and/or can help peers to learn from what is presented. Openness and transparency improves trust in results.
 
 The entry point to analysis reproducibility is to have each step documented through scripts, rather than using _point and click_ software interface. R is a very powerfull language for this:

 1. It’s open source (i.e. it can be customised to accommodate specific needs) and free to use (i.e. not only saving financial resources for core humanitarian activities but also avoiding there long and cumbersome software procurement processes);
 ...
 4. It covers the entire analysis workflow: from data import, data tidying, data reshaping, visualisation with chart, modelisation and generation of report to communicate results. 

![__Diagram:__ Data Science Workflow](data-science.png)
 
This cookbook does not aim at replacing the numerous books available on data science, like the [R for Data Science](https://r4ds.had.co.nz), it is rather a quick reference to be used by colleagues that aims at building skills in thhe area.  
...
Effective chart are first those that __support a message__. From the same dataset a multiplicity of chart can be produced. The best chart is the one that present in the most powerful way the message that you want to pass-on and the story you want to tell. 
...
You will need to first install [R](https://www.r-project.org/) and [R sudio](https://www.rstudio.com/products/rstudio/download/). The next step could be to download this repository and re-run the Rmd file line by line and see the results of each instructions to familiarise yourself with the code.
...
The idea is that you can build every graph from the same few components: a dataset (__data__), a set of geometry (__geom__ : visual marks that represents data points: bar, line, point, area, polygon, etc.) and a coordinate system (__coord__). To display data values, variables in the dataset shall be mapped to aesthetic properties of the geom (__aes__: like size, color and x and y locations). In addition, some plots visualise a transformation of the original variable that is described by the instruction __stat__.

Using scripts can appear difficult at the begining but with a bit of practice, it provides huge gain of productivity compared to point and click interactions.
...
Last Let's now download (& slighlty tidy) data from UNHCR popstat API together with indicators from the worldbank and some geodata.
 ...
 * Avoid labeling at long distance, a legend separated from the line requires the readers to do extra work cross-referencing between the key and the line. Label the lines directly. Direct labelling allows the reader to identify the lines quickly and focus on comparing and contrasting the patterns.
 ...
 * The most important line should be one colour and the other lines should be shades of second colour. 
 ...
 * Convert your data to rounded, easily digestible values for chart labelling
...
Scatterplot are used to check for correlation. A trend line can be added to compare two set of measures to determine if as one set goes up, the other set correspondingly goes up or down and how strongly.

Scatterplot are also a good way to identify clusters of observation.
